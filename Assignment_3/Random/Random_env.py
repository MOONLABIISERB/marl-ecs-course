...
class RandomMAPFEnvironment:
    def __init__(self, grid_dimension=10, wall_config=None, num_agents=4):
        ...
        # Randomizing agent and goal positions
        self.agent_pos = self._initialize_random_positions()
        self.goal_pos = [(5,8), (1,4), (4,1), (8,4)]

    def _initialize_random_positions(self):
        """Randomly place agents in the grid, avoiding walls and other agents"""
        positions = []
        while len(positions) < self.num_agents:
            x = random.randint(0, self.grid_size - 1)
            y = random.randint(0, self.grid_size - 1)
            if (x, y) not in self.walls and (x, y) not in positions:
                positions.append((x, y))
        return positions

    def initialize_scenario(self):
        """
        Set initial agent positions and goal locations to random locations
    ...
    def plot_map(self):
        ...
        plt.tight_layout()
        plt.savefig('mapf_env_random.png')
        plt.close()
...
if __name__ == "__main__":
    ...
    env.plot_map()
    print("MAPF Environment with Random Positions initialized and visualized.")

...
import time
from dynamic_env import MAPFEnv

class Agent:
    def __init__(self, num_actions=5, alpha=0.1, gamma=0.9, epsilon=0.1):
        self.q_table = {}
        self.alpha = alpha
        self.gamma = gamma
...
class MARLTrainer:
    ...
    def train(self):
        for episode in range(self.num_episodes):
            ...
            # Append episode log to training logs
    ...
    def save_q_tables(self, filename="Q_tables.pickle"):
        q_tables = {agent_id: agent.q_table for agent_id, agent in self.agents.items()}
        with open(filename, "wb") as f:
            pickle.dump(q_tables, f)
        print("Q_tables saved to", filename)

    def save_logs(self, filename="training_logs.pickle"):
        with open(filename, "wb") as f:
    ...
    def visualize_movement(self):
        ...
        for obs in self.env.obstacles:
            self.ax.add_patch(plt.Rectangle((obs[0], obs[1]), 1, 1, color='black'))

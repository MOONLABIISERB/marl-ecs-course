...
from Trainer_fixed import FixedPositionQLearningAgent, train_fixed_mapf

def find_minimum_steps_to_goal(env, agents):
    """
    Find minimum steps for each agent to reach its goal
    
    Args:
        env (FixedMAPFEnvironment): Environment instance
...
def visualize_agent_paths(env, agents, min_steps):
        """
        Visualize paths taken by agents to reach their goals
        ...
        initial_positions = [(1, 1), (8, 1), (8, 8), (1, 8)]
        for idx, (pos, goal) in enumerate(zip(initial_positions, env.goal_pos)):
            # Initial position
        ...
        plt.title('Agent Paths to Goals - Fixed Positions')
        plt.tight_layout()
        plt.savefig('fixed_mapf_agent_paths.png')
        plt.show()

if __name__ == "__main__":
    # Train agents
    trained_agents, episode_rewards, collision_counts = train_fixed_mapf()

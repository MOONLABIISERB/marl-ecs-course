...
class QLearningAgent:
    def __init__(self, n_actions, alpha=0.1, gamma=0.999):
    ...
    def choose_action(self, state):
        """Choose the best action based on the current Q-values (A greedy policy approach)."""
        state_str = str(state)
        if state_str not in self.q_table:
            self.q_table[state_str] = np.zeros(self.n_actions)  # For new state Initialize Q-values to 0 
        return np.argmax(self.q_table[state_str])  # Choose the action with the highest Q-value
...
def main() -> None:
    ...
    ep_returns = []  # Track cumulative rewards
    ...
    print(f"Average reward over {num_episodes} episodes: {np.mean(ep_returns)}")


    # Plot cumulative reward
    plt.figure(figsize=(10, 5))
    
    plt.plot(ep_returns, label='Cumulative Reward', color = 'brown')
    plt.xlabel('Episode')
    ...
    plt.figure(figsize=(10, 5))
    plt.plot(avg_losses, label='Average TD Error', color='red')
    plt.xlabel('Episode')
    plt.ylabel('TD Error')

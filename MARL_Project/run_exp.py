...
def evaluation(state, agent, eval_env, use_rl=True, use_iqn=True, act_adaptive=True, save_episode=True):
...
def exp_setup(envs, eval_schedule, i):
    ...
    for test_env in envs:
        ...
        # Ensure dynamic_env is a list and retrieve the ith value
        dynamic_env = eval_schedule["dynamic_env"][i] if isinstance(eval_schedule["dynamic_env"], list) else eval_schedule.get("dynamic_env", False)

        
        if dynamic_env:
            test_env.generate_dynamic_obstacles()  # Generate dynamic obstacles
        else:
            test_env.generate_static_obstacles()  # Generate static obstacles

        # Reset the environment and get the initial state
        state, _, _ = test_env.reset()

        # Periodically update dynamic obstacles if applicable
        if dynamic_env:
            test_env.update_dynamic_obstacles()  # Update positions of dynamic obstacles
...
if __name__ == "__main__":
    ...
    save_dir = "trained_models/IQN/training_2024-11-26-22-05-26/seed_9"
    device = "cpu"
    ...
    # save_dir = "training_data/training_2024-11-26-14-09-27/seed_9"
    save_dir = "trained_models/DQN/training_2024-11-26-22-09-57/seed_9"
    device = "cpu"
    ...
    eval_schedules = dict(num_episodes=[10,10,10,10,10],
                          num_cooperative=[2,3,4,4,5],
                          num_non_cooperative=[0,0,0,0,0],
                          num_cores=[1,2,2,3,4],
                          num_obstacles=[0,1,2,2,2],

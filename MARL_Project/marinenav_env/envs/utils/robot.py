...
class Perception:
    def __init__(self, cooperative: bool = False):
        ...
        self.angle = 2 * np.pi  # detection angle range
        self.max_obs_num = 5  # the maximum number of obstacles to be considered
        self.max_obj_num = 5  # the maximum number of dynamic objects to be considered
        self.observation_format(cooperative)
        self.observed_obs = []  # indices of observed static obstacles
        self.observed_objs = []  # indices of observed dynamic objects
...
class Robot:
    def __init__(self, cooperative: bool = False):
        ...
        self.velocity = None  # velocity with respect to sea floor
        self.start = None  # start position
        self.goal = None  # goal position
        self.collision = False
        self.reach_goal = False
    ...
    def update_dynamic_obstacles(self, dynamic_obstacles):
        # This method will update the dynamic obstacles' position based on their velocity
        for obj in dynamic_obstacles:
            if obj is not self:  # avoid updating this robot itself
                obj.x += obj.velocity[0] * self.dt
                obj.y += obj.velocity[1] * self.dt
    ...
    def check_collision(self,obj_x,obj_y,obj_r):
        ...
        if d <= 0.0:
            ...
            # print(f"Robot collision detected at position ({self.x}, {self.y})")
            return True
        return False
    ...
    #     for obj in dynamic_obstacles:
    #             if obj is self or obj.deactivated:
    #                 continue
    #             if not self.check_detection(obj.x, obj.y, obj.detect_r):
    #                 continue
    #             self.perception.observed_objs.append(j)
    #             if not self.collision:
    #                 self.check_collision(obj.x, obj.y, obj.r)
    #             if in_robot_frame:
    #                 pos_r = self.project_to_robot_frame(np.array([obj.x, obj.y]), False)
    #                 v_r = self.project_to_robot_frame(obj.velocity)
    #                 new_obs = list(np.concatenate((pos_r, v_r)))
    #                 self.perception.observation["dynamic"].append(new_obs)
    #             else:
    #                 self.perception.observation["dynamic"].append([obj.x, obj.y, obj.velocity[0], obj.velocity[1]])
    ...
    def perception_output(self, obstacles, robots, dynamic_obstacles, in_robot_frame=True):
        if self.deactivated:
            return None, self.collision, self.reach_goal
        ...
        if self.cooperative:
            ...
            # Dynamic obstacles tracking
            for i, obj in enumerate(dynamic_obstacles):
            # Check if the object has a 'deactivated' attribute, if not, always process
                if hasattr(obj, 'deactivated') and (obj is self or obj.deactivated):
                    continue
                
                if not self.check_detection(obj.x, obj.y, getattr(obj, 'detect_r', obj.r)):
                    continue
                
                self.perception.observed_objs.append(i)
        ...
        if self.collision:
            # print(f"Robot collision detected at ({self.x}, {self.y})") 
            self.deactivated = True

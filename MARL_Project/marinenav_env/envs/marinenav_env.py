...
class Obstacle:
    def __init__(self, x:float, y:float, r:float, dynamic_env:bool=False, velocity=None):
        ...
        self.velocity = np.zeros(2) if velocity is None else np.array(velocity, dtype=np.float64)
        self.trajectory = []
        # print("bool_velocity",velocity)



class MarineNavEnv2(gym.Env):
    def __init__(self,num_robots,seed: int = 0, schedule: dict = None, dynamic_env: bool = True):
        self.sd = seed
        ...
        self.dynamic_env = dynamic_env  # Flag to indicate whether dynamic obstacles should be used
        ...
        self.safe_steps = 0  
        self.max_safe_steps = 25
        self.safe_reward = 2
        if isinstance(num_robots, list):
            num_robots = num_robots[self.episode_timesteps]  # Extract based on episode index
        
        self.num_robots = num_robots  # Store as an integer
        self.safe_steps = [0] * num_robots
        ...
        self.obstacles = []  # list of obstacles (static or dynamic)
    ...
    def reset(self):
        ...
        self.safe_steps = [0] * self.num_robots
        ...
        # Generate obstacles
        if self.dynamic_env:  # For dynamic environment, create dynamic obstacles
    ...
    def generate_dynamic_obstacles(self):
        # Generate dynamic obstacles with random positions and velocities
        
        num_obs = self.num_obs
        iteration = 500  # Increased iterations for better placement
        while num_obs > 0:
            ...
            velocity = self.rd.uniform(low=3.0, high=5.0, size=2)  # Random velocity in x and y directions
            obs = Obstacle(center[0], center[1], r, dynamic_env=True, velocity=velocity)
            iteration -= 1
    ...
    def generate_static_obstacles(self):
        # Generate static obstacles with random positions and sizes
        num_obs = self.num_obs
        iteration = 500
        while num_obs > 0:
            center = self.rd.uniform(low=5.0 * np.ones(2), high=np.array([self.width - 5.0, self.height - 5.0]))
            r = self.rd.uniform(low=self.obs_r_range[0], high=self.obs_r_range[1])
            obs = Obstacle(center[0], center[1], r)  # Static obstacle by default
            iteration -= 1
            if self.check_obstacle(obs):
                self.obstacles.append(obs)
                num_obs -= 1
    ...
    def update_dynamic_obstacles(self):
        ...
        for obs in self.dynamic_obstacles:
            if obs.dynamic_env:
                ...
                new_x = obs.x + obs.velocity[0]
                new_y = obs.y + obs.velocity[1]
                
                # Boundary checking and collision response
                if new_x - obs.r < 0 or new_x + obs.r > self.width:
                    obs.velocity[0] *= -1  # Reverse x direction
                    new_x = obs.x + obs.velocity[0]
                
                if new_y - obs.r < 0 or new_y + obs.r > self.height:
                    obs.velocity[1] *= -1  # Reverse y direction
                    new_y = obs.y + obs.velocity[1]
    ...
    #     # Process robot actions and rewards as usual
    ...
    #     # Process collisions and goal achievements
    ...
    #             else:
    #                 raise RuntimeError("Robot being deactivated can only be caused by collision or reaching goal!")
    #             continue
    ...
    def step(self, actions):
        ...
        for i, action in enumerate(actions):
            ...
            # Basic rewards for timestep penalty and progress towards goal
            rewards[i] += self.timestep_penalty
            rewards[i] += dis_before - dis_after
        ...
        for idx, rob in enumerate(self.robots):
            ...
            if self.episode_timesteps >= 1000:
                ...
                rewards[idx] += self.collision_penalty  # Apply penalty for collision
                self.safe_steps[idx] = 0  # Reset safe steps on collision
                dones[idx] = True
                infos[idx] = {"state": "collision"}
            elif reach_goals[idx]:
                rewards[idx] += self.goal_reward
            ...
            # Track safe steps and apply bonus reward
            for idx, rob in enumerate(self.robots):
                if not collisions[idx]:  # Increment safe steps if no collision
                    self.safe_steps[idx] += 1
                else:  # Reset on collision
                    self.safe_steps[idx] = 0

                # Reward for staying safe
                if self.safe_steps[idx] >= self.max_safe_steps:
                    rewards[idx] += self.safe_reward
                    self.safe_steps[idx] = 0  # Reset after rewarding
    ...
    def get_observations(self):
        ...
        for robot in self.robots:
            ...
            observation, collision, reach_goal = robot.perception_output(self.obstacles, self.robots, self.dynamic_obstacles, self.observation_in_robot_frame)
    ...
    #     # Within the range of the map
    #     if self.obs.x - self.obs.r < 0.0 or self.obs.x + self.obs.r > self.width:
    #         return False
    #     if self.obs.y - self.obs.r < 0.0 or self.obs.y + self.obs.r > self.height:
    #         return False
    ...
    #         if np.linalg.norm(dis_s) < self.obs.r + self.clear_r:
    #             return False
    #         dis_g = obs_pos - robot.goal
    #         if np.linalg.norm(dis_g) < self.obs.r + self.clear_r:
    #             return False
    ...
    #         if dis <= self.r + self.obs.r:
    #             return False
        
    #     # Not collide with other obstacles
    #     for obstacle in self.obstacles:
    #         dx = obstacle.x - self.obs.x
    #         dy = obstacle.y - self.obs.y
    #         dis = np.sqrt(dx*dx + dy*dy)

    #         if dis <= obstacle.r + self.obs.r:
    #             return False
    ...
    def reset_with_eval_config(self,eval_config):
        ...
        self.obstacles.clear()
        self.dynamic_obstacles = []
        obstacles_config = eval_config["env"]["obstacles"]
        for i, position in enumerate(obstacles_config["positions"]):
            x, y = position
            r = obstacles_config["r"][i]
            dynamic_env = obstacles_config["dynamic_env"][i]
            velocity = obstacles_config["velocity"][i]
            # Trajectory = obstacles_config["trajectory"][i]
            obs = Obstacle(x, y, r, dynamic_env=dynamic_env, velocity=velocity)
            if dynamic_env:
    ...
    def episode_data(self):
        ...
        episode["env"]["obstacles"]["dynamic_env"] = []
        episode["env"]["obstacles"]["velocity"] = []
        for obs in self.obstacles:
            episode["env"]["obstacles"]["positions"].append([obs.x, obs.y])
            episode["env"]["obstacles"]["r"].append(obs.r)
            episode["env"]["obstacles"]["dynamic_env"].append(obs.dynamic_env)
            episode["env"]["obstacles"]["velocity"].append(obs.velocity.tolist() if obs.dynamic_env else [0, 0])

        episode["env"]["obstacles"]["trajectories"] = []
        for obs in self.obstacles:
            
            if obs.dynamic_env:
                episode["env"]["obstacles"]["trajectories"].append(obs.trajectory)
            else:
                episode["env"]["obstacles"]["trajectories"].append([])

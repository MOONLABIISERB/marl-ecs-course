...
import copy
import time
from marinenav_env.envs import MarineNavEnv2
...
class Trainer():
    ...
    def update_dynamic_obstacles(self):
        # Ensure that train_env is an instance of MarineNavEnv2
        if not isinstance(self.train_env, MarineNavEnv2):
            raise TypeError("self.train_env is not an instance of MarineNavEnv2.")
        
        # Call the environment's method to update dynamic obstacles
        if self.train_env.dynamic_env:  # Only update dynamic obstacles if dynamic environment is enabled
            self.train_env.update_dynamic_obstacles()
    ...
    def evaluation(self):
        ...
        for idx, config in enumerate(self.eval_config):
            ...
            while not end_episode:
                ...
                self.eval_env.update_dynamic_obstacles()  # Assuming this method exists in the environment
                ...
                for i, rob in enumerate(self.eval_env.robots):
                    ...
                    # Update rewards, times, and energies
                    if rob.cooperative:
                    ...
                    # Check for collisions or reaching goals
                    if rob.collision or rob.reach_goal:
                        rob.deactivated = True
            ...
            # Collect actions and trajectories
            actions = []
            trajectories = []

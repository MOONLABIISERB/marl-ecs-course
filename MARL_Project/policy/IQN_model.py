...
#         # Creating tau values with batch dimension handling
#         taus = torch.linspace(0.1, 0.9, num_tau).to(self.device).unsqueeze(-1)  # (num_tau, 1)
#         taus = taus.repeat(batch_size, 1, 1)  # Expand to (batch_size, num_tau, 1)

#         # Distorted quantile sampling
#         taus = taus * cvar

#         # Cosine calculation
#         cos = torch.cos(taus * self.pis)  # The resulting shape should be (batch_size, num_tau, self.n)
        
#         # Debugging print
#         # print(f"Shape of cos: {cos.shape}, Expected shape: ({batch_size}, {num_tau}, {self.n})")
        
#         # Check that the shape is correct
#         assert cos.shape == (batch_size, num_tau, self.n), f"cos shape is incorrect: {cos.shape}"
...
#         # Apply attention to dynamic features
#         dynamic_features = self.dynamic_encoder(dynamic_states)
#         dynamic_features = self.dynamic_attention(dynamic_features)

#         # Concatenate features
#         features = self.gated_fusion(self_features, static_features, dynamic_features)
...
#         features = relu(self.hidden_layer_2(features))
#         quantiles = self.output_layer(features)
#         risk_scores = self.risk_head(features)


            
#         return quantiles.view(batch_size,num_tau,self.action_size), taus ,risk_scores

...
class Agent():
    ...
    def __init__(self, 
             ...
             training=True,
             use_iqn=True,
             time_step=1.0):  # Add time_step as an optional parameter
        ...
        self.time_step = time_step  # Add this to the instance
        self.env = marinenav_env.MarineNavEnv2(seed)

        # IQN or DQN policy initialization
        if training:
            if use_iqn:
    ...
    def act(self, state, eps=0.0, cvar=1.0, use_eval=True):
        ...
        with torch.no_grad():
            ...
            # quantiles, taus = self.policy_local(state,num_tau, cvar)
    ...
    def adjust_cvar(self, state):
        """
        Adjusts the Conditional Value at Risk (CVaR) based on the state of the environment.
        """
        closest_d = float('inf')

        # Extract static and dynamic obstacle data from the state
        ...
        # Process dynamic obstacles from the environment
        if hasattr(self.env, "dynamic_obstacles"):
            for obstacle in self.env.dynamic_obstacles:
                dist = np.linalg.norm([obstacle.x, obstacle.y]) - obstacle.r - 1.6
                closest_d = min(closest_d, dist)
        else:
            # Fallback: Process dynamic obstacles from the state if the environment doesn't have dynamic obstacles
        ...
        # Scale CVaR based on the closest distance
        cvar = 1.0
        if closest_d < 10.0:
            cvar = closest_d / 10.0
    ...
    def train_IQN(self):
        ...
        Q_targets_next, taus= self.policy_target(next_states)
...
def calculate_huber_loss(td_errors, k=1.0):
  ...
  # def __init__(self, 
    ...
    # def adjust_cvar(self,state):  original_code

...
def evaluate_model_with_random_pruning_layer_by_layer(model, input_image, retain_ratio=0.5):
    ...
    with torch.no_grad():
        ...
        indices_to_retain = [0] + random.sample(range(1, num_tokens), num_to_retain - 1)
        indices_to_retain.sort()
        x = x[:, indices_to_retain]

        # Step 5: Transformer Encoder Blocks
        for i, block in enumerate(model.blocks):
            x = block(x)

        # Step 6: Layer Normalization before Classification Head
        x = model.norm(x)
...
def evaluate_model_with_pruning_metrics_manual_flops(model, test_loader, image_size, patch_size, embed_dim, num_classes, retain_ratio=0.5):
    ...
    progress_bar = tqdm(test_loader, desc="Testing with Pruning")

    with torch.no_grad():
        for inputs, labels in progress_bar:
            inputs, labels = inputs.to(device), labels.to(device)
            outputs = evaluate_model_with_random_pruning_layer_by_layer(model, inputs, retain_ratio=retain_ratio)

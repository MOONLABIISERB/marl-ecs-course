...
class PressurePlate(gym.Env):
    ...
    def __init__(self, height, width, n_agents, sensor_range, layout):
        ...
        # Add a new attribute to track if agents have reached their goals
        self.agents_at_goal = [False] * n_agents

    def step(self, actions):
        np.random.shuffle(self.agent_order)

        for i in self.agent_order:
            # Skip movement if agent is already at its goal
            ...
            if i == len(self.agents) - 1:  # Last agent checks for final goal
                if [self.agents[i].x, self.agents[i].y] == [self.goal.x, self.goal.y]:
                    self.agents_at_goal[i] = True
            else:  # Other agents check for their pressure plates
                if [self.agents[i].x, self.agents[i].y] == [self.plates[i].x, self.plates[i].y]:
                    self.agents_at_goal[i] = True

        # Update plates and doors
    ...
    def reset(self):
        # Reset the agents_at_goal tracker
        self.agents_at_goal = [False] * self.n_agents
    ...
    def _get_rewards(self):
        ...
        for i, agent in enumerate(self.agents):
            # If agent is at its goal, give it maximum reward
            if self.agents_at_goal[i]:
                rewards.append(0.0)  # Maximum reward (0 since rewards are negative)
                continue

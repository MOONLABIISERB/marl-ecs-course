...
class MAPFEnv:
    ...
    def plot_map(self):
        ...
        plt.savefig('EnvironmentGrid.png')
...
def train(seed: int) -> tuple:
    mode = input("Enter mode (random or None): ").strip()
    ...
    for episode in range(n_episodes):
        ...
        # Rollout loop
        for rollout in range(10):  # Number of rollouts per episode
    ...
    plt.ylabel('Reward')
    plt.legend()
    plt.savefig('agent_rewards_with_rollouts.png')
    plt.show()
...
def find_min_steps_for_all_agents(env: MAPFEnv, agents: list) -> np.ndarray:
    ...
    for i, agent in enumerate(agents):
        ...
        while agent_position != env.goal_pos[i]:
...
def save_q_values(agents, filename='q_values_with_rollouts.npz') -> None:
    q_values = np.array([agent.q_table for agent in agents])
    np.savez(filename, q_values=q_values)


save_q_values(agents)
env.plot_map()

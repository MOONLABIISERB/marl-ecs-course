...
#     # To store cumulative rewards for plotting
#     episode_rewards = []
...
#         state = tuple(state)  # Convert state to a tuple to use as key for Q-table
#         total_reward = 0
...
#             best_next_action = np.argmax(Q[next_state])  # Get best next action from Q-table
...
#         if (ep + 1) % 100 == 0:
#             print(f"Episode {ep + 1}/{num_episodes}, Total Reward: {total_reward}")

#     # Plotting Episode vs Cumulative Reward
...
class ModTSP(gym.Env):
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict[str, None]]:
        ...
        if self.visited_targets[next_loc] == 1:  # If visiting an already visited target
            reward = -1e4  # Give a heavy negative reward
        else:
            self.current_profits -= self.distances[past_loc, next_loc]
            reward = self._get_rewards(next_loc)
            self.visited_targets[next_loc] = 1
    ...
    def _get_rewards(self, next_loc: int) -> float:
        ...
        Returns:
            float: Reward based on the travel distance, or negative reward if revisiting an old target.
        """
        reward = self.current_profits[next_loc] if not self.visited_targets[next_loc] else -1e4
        return float(reward)
...
def main():
    ...
    def epsilon_greedy_action(state, epsilon):
        """Choose action based on epsilon-greedy policy, avoiding revisiting old positions."""
        valid_actions = np.where(env.visited_targets == 0)[0]  # Select only unvisited targets

        if np.random.random() < epsilon:
            return np.random.choice(valid_actions)  # Explore (only from valid actions)
        else:
            # Exploit, choosing from valid actions only
            q_values = np.array([Q[state][a] if a in valid_actions else -1e10 for a in range(env.action_space.n)])

...
if __name__ == "__main__":
    ...
    Q_first = np.zeros((env.num_targets, env.num_targets))
    Q_every = np.zeros((env.num_targets, env.num_targets))
    return_first = defaultdict(list)  
    return_every = defaultdict(list)  
        ...
        for episode in range(num_episodes):
            ...
            while not done:
                ...
                if np.random.random() < epsilon:
                    ...
                    action = np.argmax(Q_first[state]) if method == "first visit" else np.argmax(Q_every[state])
            ...
            for state, action, reward in reversed(episode_history):
                G = reward + disc_factor * G
                
                if method == "first visit":
                    # First-visit MC
                    if (state, action) not in return_first:  # Check if first visit
                        return_first[(state, action)].append(G)
                        Q_first[state, action] = np.mean(return_first[(state, action)])  

                elif method == "every visit":
                    # Every-visit MC
                    return_every[(state, action)].append(G)
                    Q_every[state, action] = np.mean(return_every[(state, action)]) 

            if episode % 100 == 0:
                print(f"[{method}] Episode {episode}, Total reward: {total_reward}")

    # Extract the optimal policy after convergence
    policy_first = np.argmax(Q_first, axis=1)
    policy_every = np.argmax(Q_every, axis=1)

    print("Optimal Policy (First Visit):", policy_first)
    print("Optimal Policy (Every Visit):", policy_every)
